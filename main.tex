...
\item Video modeling on \textsc{CrossTask} \footcite{zhukov2019cross}
    \begin{itemize}
    \item Scale state with fixed computation budget vs accuracy
...
\begin{frame}
\frametitle{Model 2: Probabilistic Context-Free Grammars (PCFG)}
...
We wish to maximize
$$p(x) = \sum_{\text{tree}: \text{yield}(\text{tree}) = x} p(\text{tree})$$
\end{frame}
...
\vspace{1em}
\item For each span $(i,k)$, sum over split points $j \in (i,k)$:
