...
A context-free grammar $\mcG = (S,\mcN,\mcP,\mcX,\mcR)$ where
\begin{align*}
    S:& \text{ Start symbol}\\
    \mcN:& \text{ Nonterminal symbols}\\
    \mcP:& \text{ Preterminal symbols}\\
    \mcX:& \text{ Terminal symbols}\\
    \mcR:& \text{ Rules} 
\end{align*}
...
\begin{itemize}
\item Assign probability mass to each rule
$$p(r = A \to B\ C)$$
\item Joint distribution over rules in tree $t$
$$p(t) = \prod_{r\in t} p(r)$$
\item Inference requires summing over all trees
